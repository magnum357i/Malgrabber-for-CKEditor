	...
	try {
		...
		if ( $ctval > $config["cookie_limit"] )
		{ throw new Exception( $config["cookie_limit"] . ',' . $cookie_hours . ',' . $cookie_mins ); }
			...
			elseif ( !is_dir($base_path . $config["image_save_path"]) )
			{ throw new Exception('poster folder is not created'); }
				...
				if ( $image_width > $config["image_max_size"] and $image_width > $config["image_max_size"] )
