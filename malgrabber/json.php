	...
	try {
		...
		{ throw new Exception( cookie_limit . ',' . $cookie_hours . ',' . $cookie_mins ); }
			...
			{ throw new Exception( 'poster folder is not created' ); }
				...
				$displayed_path = $_SERVER['HTTP_HOST'] . '/' . $displayed_path;

					if ( https == "on" ) { $displayed_path = "https://" . $displayed_path; }
					elseif ( https == "off" ) { $displayed_path = "http://" . $displayed_path; }
				...
				$json =
				array(
				'mal_created_date'      => date('/d-m-Y/H:i/w/', time()),
				'displayed_poster_path' => $displayed_path . image_save_path,
